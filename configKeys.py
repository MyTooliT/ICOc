    ...
    def atXmlConfigurationSth(self):
        ...
        for tSetup in self.root.find('Setup'):
            if "ProductionSth" == tSetup.get('name'):               
                ...
                self.sLogLocationSth = str(tSetup.find('VerificationInternal').find('LogLocation').text)
                    ...
                    if self.sUsedHwSth == tHwRev.get('name'):
                        for tHwRevSub in tHwRev.find('HwRevSub'):
                            if self.sUsedHwSubSth == tHwRevSub.get('name'):
                                atDbData["sVersionSth"] = str(tHwRevSub.find('Version').text)
                                atDbData["sLogNameSth"] = str(tHwRevSub.find('LogName').text)
                                atDbData["sLogLocationSth"] = str(tHwRevSub.find('LogLocation').text)
                                atDbData["sOtaComPortSth"] = str(tHwRevSub.find('OtaComPort').text)
                                atDbData["sBuildLocationSth"] = str(tHwRevSub.find('BuildLocation').text)
                                atDbData["sSilabsCommanderLocatonSth"] = str(tHwRevSub.find('SilabsCommanderLocaton').text)
                                atDbData["sAdapterSerialNoSth"] = str(tHwRevSub.find('AdapterSerialNo').text)
                                atDbData["sBoardTypeSth"] = str(tHwRevSub.find('BoardType').text)
                                atDbData["iSensorAxisSth"] = int(tHwRevSub.find('SensorAxis').text)
                                atDbData["iAdc2AccSth"] = int(tHwRevSub.find('Adc2Acc').text)
                                atDbData["sKeySth"] = str(tHwRevSub.find('Key').text)  
        return atDbData
    ...
    def atXmlConfigurationStu(self):
        ...
        for tSetup in self.root.find('Setup'):
            if "ProductionStu" == tSetup.get('name'):                  
                ...
                self.sLogLocationStu = str(tSetup.find('VerificationInternal').find('LogLocation').text)
                for tHwRev in tSetup.find('HwRev'):
                    if self.sUsedHwStu == tHwRev.get('name'):
                        for tHwRevSub in tHwRev.find('HwRevSub'):
                            if self.sUsedHwSubStu == tHwRevSub.get('name'):
                                atDbData["sVersionStu"] = str(tHwRevSub.find('Version').text)
                                atDbData["sLogNameStu"] = str(tHwRevSub.find('LogName').text)
                                atDbData["sLogLocationStu"] = str(tHwRevSub.find('LogLocation').text)
                                atDbData["bPcbOnlyStu"] = bool(tHwRevSub.find('PcbOnly').text)
                                atDbData["sBuildLocationStu"] = str(tHwRevSub.find('BuildLocation').text)
                                atDbData["sSilabsCommanderLocatonStu"] = str(tHwRevSub.find('SilabsCommanderLocation').text)
                                atDbData["sAdapterSerialNoStu"] = str(tHwRevSub.find('AdapterSerialNo').text)
                                atDbData["sBoardTypeStu"] = str(tHwRevSub.find('BoardType').text)
                                atDbData["sKeyStu"] = str(tHwRevSub.find('Key').text)
        return atDbData
