...
check:
	...
	mypy mytoolit
	pylint -E mytoolit
	pylint mytoolit --ignore-patterns '(extended_test_(result|runner)|icon|forms|message|network|node|open|parse|pdf|report|smh|status|storage|sth|stu|streaming|units)\.py'
